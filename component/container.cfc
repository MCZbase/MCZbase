...
<cffunction name="getContDetails" access="remote">
	...
	<cftry>
		<cfquery name="queriedFor" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#" timeout="60">
			SELECT
				CONTAINER_ID,
				PARENT_CONTAINER_ID,
				CONTAINER_TYPE,
...
<cffunction name="get_containerContents" access="remote">
	...
	<cftry>
		<cfquery name="result" timeout="60" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
			SELECT
				CONTAINER_ID,
				PARENT_CONTAINER_ID,
				CONTAINER_TYPE,
...
<cffunction name="get_containerTree" access="remote">
	...
	<cfset thisSql = "
				...
				label || decode(get_storedas_by_contid(container_id), null, null, ' <br>&nbsp;Stored as: <em>' || get_storedas_by_contid(container_id)) || '</em>' label,
			...
			">
		 	...
		 	<cfif #queriedFor.recordcount# is 0>
				...
				<cfset i=1>
				<cfset result = querynew("CONTAINER_ID,PARENT_CONTAINER_ID,LABEL,STOREDAS,CONTAINER_TYPE")>
	  			<cfloop query="ro">
	  				<cfif not listfind(alreadyGotOne,CONTAINER_ID)>
						...
						<cfset temp = QuerySetCell(result, "storedAS", "#storedAS#", #i#)>
