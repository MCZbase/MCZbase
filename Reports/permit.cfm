...
<cfif #action# is 'Generate the PDF'>
	<cfif not isdefined('permit_id')>
	   <cfthrow message="No permit_id value provided when generating a PDF permit report for a permit_id">
		<cfabort>
	</cfif>
	...
	<cfdocument
	        ...
	        marginright=".25"
			unit="in"
	        orientation="landscape" filename="#Application.webDirectory#/temp/permit_#cookie.cfid#_#cookie.cftoken#.pdf" overwrite="true">
	...
	<a href="/temp/permit_#cookie.cfid#_#cookie.cftoken#.pdf">Get the PDF</a><br />
	</cfoutput>
...
<!--- validate permit_num --->
	...
	<cfquery name="getPermitID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
		#preservesinglequotes(sql)#
	</cfquery>
	<!--- debug code
...
<cfset sql="          
			...
			where 
				...
				#order#"
>
<cfquery name="specimens" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
	#preservesinglequotes(sql)#
</cfquery>
