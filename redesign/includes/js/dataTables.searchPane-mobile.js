...
(function(factory) {
	...
	$.extend(SearchPanes.prototype, {
		...
		_clear: function(pane) {
			...
			pane.find('option.' + itemSelected).removeClass(itemSelected);
			...
			for (var i = 0, ien = data.length; i < ien; i++) {
				if (data[i]) {
					var li = $('<option/>')
						...
						.html(
							'<span class="' + itemClasses.label + '" style="padding-right: 1em;">' + data[i] + '</span>'
						)
						.data('filter', data[i])
						.append(
							$('<span/>')
		...
		_toggle: function(li) {
			...
			var filters = pane.find('option.' + itemSelected);
