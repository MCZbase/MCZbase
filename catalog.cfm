...
<cfquery name="identifiers" dbtype="query">
    ...
    <cfquery name="ctns" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#"  cachedwithin="#createtimespan(0,0,60,0)#">
		...
		<cfquery name="cPreserveMethod" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#"  cachedwithin="#createtimespan(0,0,60,0)#">
			SELECT distinct(preserve_method) FROM ctSpecimen_preserv_method
				WHERE collection_cde='#detail.collection_cde#'
				order by preserve_method
        </cfquery>
		<cfquery name="cNumericModifiers" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#"  cachedwithin="#createtimespan(0,0,60,0)#">
