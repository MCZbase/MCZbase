...
<ul>
	...
	<li><strong>|</strong> or <em>Louis | Alexander</em></li>
	<li><strong>,</strong> weighted or (accumulate) <em>Louis, Alexander, Agassiz</em></li>
	<li><strong>~</strong> not <em>~brazil</em></li>
	<li><strong>!</strong> sounds like <em>!louis</em></li>
	<li><strong>fuzzy(term)</strong> fuzzy match <em>fuzzy(Agassiz)</em></li>
	<li><strong>-</strong> rank lower <em>-brazil</em></li>
...
<!-------------------------------                d              --------------------------------------->
<cfif #content# is "search_help">
	<cfset title="Search Help">
<h2>MCZbase Search Tips</h2>
<p>
Search fields are case-insensitive. Use any combination of fields to return specimen results. Click the <strong>"Show More Options"</strong> text in the corner of each section to display advanced search fields and narrow query results to a finer resolution.
</p><p>
Include diacritic marks in text strings for names and localities that have accents.
</p><p>
Enter wildcard symbols to expand the scope of a search or when uncertain of spelling:
</p><p>
Underscore (_) will match any single character (e.g., "Cl_ck" retrieves "Clack," "Click," "Clock,")
</p><p>
...
Isolate specimen records with tissue samples by toggling the <strong>"Requires Tissues?"</strong> checkbox at the top of the Specimen Search page.
</p><p>
As an alternative to text-based locality searches, perform spatial queries using the <strong>"Select on Google Map"</strong> link in the Locality section. Pan and zoom to identify a desired search area, or use the Spatial Query Tool   to delineate the precise boundaries of the area by dragging the bottom right corner to adjust the size of the frame. Coordinates ranges encompassing the search area will be displayed at the top of the map.
</p>
<h2>Display Options</h2>
<p>
Indicate search results display preference using the <strong>"See Results as"</strong> drop-down menu at the top of the Specimen Search page:
</p>
<dl>
<dt>Specimen Records</dt><dd>displays results in a traditional table list format
</dd>
HTML Specimen Records</dt><dd>displays results in HTML structure with graduated levels of detail.</dd>
<dt>BerkeleyMapper Map</dt><dd>displays results as an interactive point distribution map</dd>
<dt>KML</dt><dd>displays results on an interactive Google Earth viewer</dd>
<dt>Specimen Summary</dt><dd>displays counts of specimen records based on selected groupings</dd>
<dt>Graph</dt><dd>displays results in a 2D or 3D chart based on specified parameters</dd>
</dl>
<p>
Sort search results at multiple levels using the <strong>"Order by"</strong> drop-down menus found at the top of the Search Results page (arrows indicate ascending and descending order).
</p><p>
Customize search result display details by clicking the <strong>"Customize Form"</strong> button to checkmark preferences.
</p><p>
Download a .csv file of queried specimen records by clicking the <strong>"Download"</strong> button at the top of the Search Results screen and agreeing to the data use terms and conditions.
</p><p>
...
<!----------------------------------------------------------------------------------------------------->
<cfif #content# is "diacritics">
<cfset title="Diacritics and Extended Characters in Searches Help">
<p>Throughout MCZbase, the majority of search fields are case-insensitive and use fuzzy matching in order to retrieve relevant and comprehensive results. However, in order to obtain results that best conform to entered search criteria, use proper spelling, case, and punctuation when possible, including diacritic letters (umlaut, tide, accent marks, etc.).  In cases where diacritic marks are incorrectly omitted from a query, results will be limited to records that do not contain these symbols.</p>
<h2>Search Tips</h2>
...
<p>When uncertain of spelling, enter wildcard symbols to expand the scope of a search:</p>
<ul>
<li>Underscore (_) will match any single character (e.g., "Cl_ck" retrieves "Clack," "Click," "Clock," "Cluck").a</li>
<li>Percent sign (%) will match a text string of any length (e.g., "Alb%" retrieves "Albany," "Albuquerque," "Albania," "Albäcksskolan")</li>
</ul>

<h2>Common Diacritic Letter Codes</h2>
<p>In the Windows system, hold down the ALT key while typing the desired ascii extended character code on the numeric keypad on your keyboard to create diacritic characters.</p>
<p>In Gnome, add combining diacritics with the base character then Ctrl+Shift+U  then the number from the Unicode range 0×300 to 0×315 for the desired diacritic.  For example, a Ctrl+Shift+U 301 produces &aacute;.</p>
<p>For copy and paste: À�?ÂÃÄÅÆ ÇÈÉÊËÌ�?Î�?�?Ñ ÒÓÔÕÖØ ÙÚÛÜ�?ß àáâãäåæ çèéêëìíîïñ òóôõöøùúûüýÿ Ā�?ĂăĄą ĆćĈĉĊċČ�? Ď�?�?đĒēĔĕĖėĘęĚě Ĝ�?ĞğĠġĢģ ĤĥĦħĨĩĪīĬĭĮįİıĲĳĴĵ ĶķĹĺĻļĽľĿŀ�?łŃńŅņŇňŉ Ō�?Ŏ�?�?őŒœ ŔŕŖŗŘřŚśŜ�?ŞşŠš ŢţŤťŦŧ ŨũŪūŬŭŮůŰűŲų ŴŵŶŷŸŹźŻżŽž ſƒƠơƯư �?ǎ�?�?ǑǒǓǔǕǖǗǘǙǚǛǜ ǺǻǼǽǾǿ</p>
</cfif>
<!----------------------------------------------------------------------------------------------------->
...
<cfset title="Geographic Element Help">
	...
	<ul>
			...
			MCZbase records the following fields:
	<ul>
...
Search for any part of the full taxon name (ie, family or subspecies). MCZbase taxonomy records:
<ul>
	<li>Class</li>
	<li>Order</li>
	<li>Suborder</li>
...
<cfif #content# is "map">
	...
	There are currently two mapping applications with which to map MCZbase specimen records. Both are JAVA applications, and both are known to have problems with some browsers/platforms.
	<p>
