...
<cffunction name="getWikiArticle" access="remote" returntype="string" output="false" returnFormat="plain">
	...
	<cfset var url = "https://code.mcz.harvard.edu/wiki/api.php?action=parse&page=#URLEncodedFormat(arguments.pageTitle)#&section=#URLEncodedFormat(arguments.section)#&prop=text&format=json">
	...
	<cftry> 
		...
		<div id="overlaycontainer" style="position: relative;"> 
			...
			<form id='findCollectingEventForm' >
				<input type='hidden' name='method' value='getCollectingEvents'>
				<input type='hidden' name='returnformat' value='plain'>
				<input type='hidden' name='target_id' value='#target_id#'>
...
<!---
   Function to store the serialization of grid column order for a user and page.
   @param page the path and filename of the page on which the grid appears.
   @param column_order json serialization of a column order map.
	@param label an optional user supplied label for the settings for that page.
 --->
<cffunction name="saveGridColumnOrder" returntype="query" access="remote">
	<cfargument name="page_file_path" required="yes">
	<cfargument name="column_order" required="yes">
	<cfargument name="label" required="no" default="Default">
		...
		<cfif exists.ct EQ 0>
			<cfquery name="insert" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#" result="insert_result">
				insert into cf_grid_properties (
					...
					<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#session.username#">,
					<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#label#">,
					<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#column_order#"> 
				)
			</cfquery>
		<cfelse>
			<cfquery name="update" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#" result="update_result">
...
<cffunction name="getGridColumnOrder" returntype="any" returnformat="json" access="remote">
	<cfargument name="page_file_path" required="yes">
	<cfargument name="label" required="no" default="Default">
