			...
			$(document).ready(function() {
				...
				$('##searchForm').bind('submit', function(evt){
					...
					$("##searchResultsGrid").on("bindingcomplete", function(event) {
						...
						$('##resultLink').html('<a class="py-1" href="/Taxa.cfm?execute=true&' + $('##searchForm :input').filter(function(index,element){ return $(element).val()!='';}).serialize() + '">Link to this search</a>');
