...
$(document).ready(function(){
	...
	//this is from https://stackoverflow.com/questions/16183231/jquery-append-and-remove-dynamic-table-row  
$(document).ready(function(){
	$(".addAgent").click(function(){$("##customAgent").append('<div class="row mt-2"><label for="other_id" class="sr-only float-left">Agent X</label><div class="col-lg-6 px-1 float-left"><select class="data-entry-select"><option value="">Collector</option><option value="1">Preparator</option></select></div><div class="col-12 col-xl-5 px-1 float-left"><input type="text" class="data-entry-input" name="agent" placeholder="Value"></div><button href="javascript:void(0);" arial-label="remove" class="btn addAgent float-left data-entry-button px-0 remAgent" style="width:20px;"><i class="fas fa-times"></i></button></div>');
	});
