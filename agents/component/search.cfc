	...
	<cftry>
		...
		<cfif isdefined("collected_date") and len(#collected_date#) gt 0>
			...
			<cfif len(#collected_date#) LT 10 OR len(#to_collected_date#) LT 10>
				<cfquery name="lookupdate" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#" result="search_result" timeout="#Application.short_timeout#">
		...
		<cfif isdefined("birth_date") AND len(#birth_date#) GT 0 AND NOT birth_date IS "NULL" AND NOT birth_date IS "NOT NULL" >
			...
			<cfif len(#birth_date#) LT 10 OR len(#to_birth_date#) LT 10 >
				<cfquery name="lookupbdate" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#" result="search_result" timeout="#Application.short_timeout#">
		...
		<cfif isdefined("death_date") and len(#death_date#) gt 0 AND NOT death_date IS "NULL" AND NOT death_date IS "NOT NULL">
			...
			<cfif len(#death_date#) LT 10 OR len(#to_death_date#) LT 10>
				<cfquery name="lookupddate" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#" result="search_result" timeout="#Application.short_timeout#">
...
<cffunction name="getPreferredNameExists" access="remote" returntype="any" returnformat="json">
	...
	<cfset retval ="">
	<cftry>
		<cfquery name="search" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#" result="search_result" timeout="#Application.short_timeout#">
