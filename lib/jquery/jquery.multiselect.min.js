	...
	d.widget("ech.multiselect", {
		...
		refresh: function (a) {
			...
			b.find("option").each(function (b) {
				...
				e += '<input id="' + b + '" name="multiselect_' + i + '" type="' + (c.multiple ? "checkbox" : "radio") + '" value="' + k + '" class="mr-1" title="' + f + '"';
		...
		_bindEvents: function () {
			...
			var b = this,
				c = this.button;
			c.find("span").bind("change.multiselect", a);
			c.bind({
				change: a,
				keypress: function (a) {
					switch (a.which) {
			...
			this.header.delegate("a", "change.multiselect", function (a) {
				if (d(this).hasClass("ui-multiselect-close")) b.close();
				else b[d(this).hasClass("ui-multiselect-all") ? "checkAll" : "uncheckAll"]();
				a.preventDefault()
			});
			this.menu.delegate("li.ui-multiselect-optgroup-label a", "change.multiselect", function (a) {
				a.preventDefault();
				var c = d(this),
				...
				switch (a.which) {
					...
					case 39:
						...
						break;
					case 13:
						d(this).find("input")[0].change()
				}
			}).delegate('input[type="checkbox"], input[type="radio"]', "change.multiselect", function (a) {
				var c = d(this),
					g = this.value,
					e = this.checked,
					i = b.element.find("option");
				this.disabled || !1 === b._trigger("change", a, {
					value: g,
					text: this.title,
					checked: e
