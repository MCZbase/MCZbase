...
// Popup a taxon picklist, retaining the value of taxonNameFld if a match is not found
// For use with data entry form, allowing entry of hybrids (which can be correctly split by the bulkloader into two taxa from the single field).
function taxaPickOptional(taxonIdFld,taxonNameFld,formName,scientificName){
	var url="/picks/TaxaPick.cfm";
	var taxonIdFld;
	var taxonNameFld;
	var formName;
	var scientificName;
	var popurl=url+"?keepValue=1&taxonIdFld="+taxonIdFld+"&taxonNameFld="+taxonNameFld+"&formName="+formName+"&scientific_name="+scientificName;
	taxapick=window.open(popurl,"","width=400,height=338, resizable,scrollbars");
}
