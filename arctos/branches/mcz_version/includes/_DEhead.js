...
/**
 * Moved into DSajax.js in Arctos Trunk. 
 * 
 * @param OrigUnits
 * @return
 */
...
/**
 * Set the the value for all of the part_lot_count_x fields to 1.
 * 
 * @depreciated  This should be a candidate for removal, 
 *     it's functionality appears to have been removed from the Arctos trunk.  
 *       
 * @param force if true, set all the part_lot_count_x fields to 1, 
 *      if false, retain existing values greater than 1.
 */
function setDefaultPartLotCountValues(force) {
	...
	for (i=0;i<partLotCount.length;i++) {
        ...
        // maintain existing values if they are greater than 1
        if (thisVal<=1 || force==true) {
...
function changeMode (mode,collobjid) {
	...
	var MINIMUM_NON_TEMPLATE_COLLOBJECTID = 20;
	
	if (mode == 'edit') {
		// don't let them edit the templates
		if (collobjid < MINIMUM_NON_TEMPLATE_COLLOBJECTID) {
		...
		if (isGoodSave > 0) {
			...
			pgClr.style.backgroundColor = '#FF6EC7';  // FF6EC7 = Purplish pink
		} else {
			// good save
			//pgClr.style.backgroundColor = '#00CCCC';  // OOCCCC=Turquoise
			pgClr.style.backgroundColor = '#B2F8F9';  // B2F8F9=Pale blue
		...
		if (isGoodSave > 0) {
			// bad save, loadedmsg contains something besides spaces
			pgClr.style.backgroundColor = '#669999';  // 669999=Greyish dark blue
			// clear defaults
		} else {
			// good save
			pgClr.style.backgroundColor = '#bed88f';  // BED88F = light olive green 
			tDiv.style.display='none';
	...
	splashPg.style.display='none';   // hide the splash screen message
	pgClr.style.display='';	         // turn on the data entry screen (unhide theTable)
}
