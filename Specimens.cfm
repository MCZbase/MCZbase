					...
					<div class="tabs card-header tab-card-header px-2 pt-3">
						...
						<div class="tab-content mt-0 px-0 pb-0">
										...
										<div class="container-flex togglesearchform" style="display: block;">
											<div class="col-12 search-form-basic-odd px-0 pb-2">
												...
												<cfif listFind(searchPrefList,"IDDetail") GT 0 OR hiddenHaveValue>
													<cfset IDDetailStyle="">
													<cfset toggleTo = "0">
														<cfset IDButton = "Identifiers&thinsp;<i class='fas fa-caret-up' title='fewer fields'><span class= 'sr-only'>fewer fields</span></i>">
												<cfelse>
													<cfset IDDetailStyle="display:none;">
													<cfset toggleTo = "1">
													<cfset IDButton = "Identifiers&thinsp;<i class='fas fa-caret-down' title='fewer fields'><span class= 'sr-only'>fewer fields</span></i>">
												</cfif> 
												...
												<div class="col-12 col-xl-2 px-1 pt-1 mb-1 border float-left" style="-webkit-box-shadow: -1px 15px 12px -4px rgba(176,170,176,0.55);-moz-box-shadow: -1px 15px 12px -4px rgba(176,170,176,0.55);box-shadow: -1px 15px 12px -4px rgba(176,170,176,0.55);">
													<div style="font-size: 0.85rem;" class="h6 font-weight-bold d-inline-block-md text-xl-right px-1 w-100 text-left text-md-left text-dark mb-1 mb-md-0 pt-1 pt-md-0">
														...
														<cfif findNoCase('redesign',gitBranch) GT 0 OR (isdefined("session.roles") AND listfindnocase(session.roles,"collops") ) >
															<button type="button" id="IDDetailCtl" class="btn btn-xs px-1 pt-xl-1 px-xl-1 small py-0 border-0 btn-link" onclick="toggleIDDetail(#toggleTo#);">#IDButton#</button>
														</cfif>
													</div>
												</div>	
							...
							function toggleIDDetail(onOff) {
								if (onOff==0) {
									$("##IDDetail").hide();
									$("##IDDetailCtl").attr('onCLick','toggleIDDetail(1)').html('<i class="fas fa-caret-down"><span title="more fields" class="sr-only">more fields</span></i>');
								} else {
									$("##IDDetail").show();
									$("##IDDetailCtl").attr('onCLick','toggleIDDetail(0)').html('<i class="fas fa-caret-up"><span title="fewer fields" class="sr-only">fewer fields</span></i>');
								}
							...
							function toggleGeogDetail(onOff) {
								if (onOff==0) {
									$("##GeogDetail").hide();
									$("##GeogDetailCtl").attr('onCLick','toggleGeogDetail(1)').html('<i class="fas fa-caret-down"><span class="sr-only">show more fields</span></i>');
								} else {
									$("##GeogDetail").show();
									$("##GeogDetailCtl").attr('onCLick','toggleGeogDetail(0)').html('<i class="fas fa-caret-up"><span class="sr-only">show fewer fields</span></i>');
								}
								<cfif isdefined("session.username") and len(#session.username#) gt 0>
									jQuery.getJSON("/specimens/component/search.cfc",
										{
							...
							function toggleCollDetail(onOff) {
								if (onOff==0) {
									$("##CollDetail").hide();
									$("##CollDetailCtl").attr('onCLick','toggleCollDetail(1)').html('<i class="fas fa-caret-down"><span class="sr-only">show more fields</span></i>');
								} else {
									$("##CollDetail").show();
									$("##CollDetailCtl").attr('onCLick','toggleCollDetail(0)').html('<i class="fas fa-caret-up"><span class="sr-only">show fewer fields</span></i>');
								}
								<cfif isdefined("session.username") and len(#session.username#) gt 0>
									jQuery.getJSON("/specimens/component/search.cfc",
										{
							...
							function toggleSpecDetail(onOff) {
								if (onOff==0) {
									$("##SpecDetail").hide();
									$("##SpecDetailCtl").attr('onCLick','toggleSpecDetail(1)').html('<i class="fas fa-caret-down"><span class="sr-only">show more fields</span></i>');
								} else {
									$("##SpecDetail").show();
									$("##SpecDetailCtl").attr('onCLick','toggleSpecDetail(0)').html('<i class="fas fa-caret-up"><span class="sr-only">show fewer fields</span></i>');
								}
								<cfif isdefined("session.username") and len(#session.username#) gt 0>
									jQuery.getJSON("/specimens/component/search.cfc",
										{
