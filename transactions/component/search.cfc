	...
	<cftry>
		...
		<cfquery name="search" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#" result="search_result">
			SELECT 
				...
				concattransagent(trans.transactionid,'entered_by') as ent_agent
			FROM 
			...
			WHERE
				...
				<cfif isDefined("agent_1") and len(agent_1) gt 0>
					and concattransagent(trans.transactionid,<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#agent_role_1#">)
