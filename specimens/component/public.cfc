		...
		<cfoutput>
			<cftry>
				...
				<div class="col-7 px-0 float-left">
					...
					<cfif listcontainsnocase(session.roles,"manage_specimens")>
						<div class="w-75 m-2">
							<a type="button" class="btn btn-xs btn-powder-blue mx-2 float-left pt-0" href="/SpecimenResults.cfm?geog_auth_rec_id=#getloc.geog_auth_rec_id#">Higher Geography</a>
							<a type="button" class="btn btn-xs btn-powder-blue mx-2 float-left pt-0" href="/SpecimenResults.cfm?locality_id=#getloc.locality_id#">Specific Locality</a>
						</div>
					</cfif>
				</div>
				...
				<cfif colls.recordcount gt 0>
					<cfif colls.recordcount eq 1>
						<li class="list-group-item pt-0"><h5 class="my-0 d-inline">Collector:&nbsp;</h5>
							<cfloop query="colls">
								<cfif preferred_agent_name.agent_name != '[no agent data]'>
									<a href="/agents/Agent.cfm?agent_id=#colls.agent_id#"></cfif>#colls.collectors#<cfif preferred_agent_name.agent_name != '[no agent data]'></a>
								</cfif>
							</cfloop>
						</li>
					<cfelse>
