...
<!--- given a string representing a json fragment in the form
  "openParens":"0","closeParens":"0" set closeParens to 0, and return 
   the string.
  @param nest string containing the openParens, closeParens fragment.
  @return the input nest string with closeParens set to 0, or
    if unable to recgnise the string or split out the numeric bits, the
    original input string without alteration.
--->
...
<cffunction name="constructJsonForField">
	...
	<cfelseif CompareNoCase(dataType,"NUMERIC") EQ 0 AND (ArrayLen(REMatch("^[0-9]+\-[0-9]+$",value)) GT 0) >
		<!--- ScriptNumberListToJson field paramter is just the field, not the json field:fieldname clause in the local field variable. --->
...
<cffunction name="executeFixedSearch" access="remote" returntype="any" returnformat="json">
	...
	<!--- Handle nesting of other number type and other number value, with posible or clause for other number 1/other number type 1 --->
	<cfset nest = '"openParens":"0","closeParens":"0"'>
	<cfset has0 = false>
	<cfset has1 = false>
	...
	<cfif has0 AND has1>
		<!--- create nested or clause has ((other_id_type and other_id_number) or (type_1 and other_id_number_1)) --->
		<!--- First Clause --->
		<!--- openParens+1 for outermost ( --->
		<!--- openParens+1 for first (type and number) clause --->
		<cfset nest = '"openParens":"2","closeParens":"0"'>
		<cfset openWith = 2>
		<cfset closeWith = 1><!--- for first (type and number) clause --->
		<cfif isDefined("other_id_type") AND len(other_id_type) GT 0>
			<cfif isDefined("other_id_number") AND len(other_id_number) GT 0>
				<cfset nest = '"openParens":"2","closeParens":"0"'><!--- open outer and inner, don't close either --->
			<cfelse>
				<!--- there is only a type in the first clause --->
				<cfset nest = '"openParens":"2","closeParens":"1"'><!--- open outer and inner, close inner --->
			</cfif>
			...
			<cfset separator = ",">
			<cfset join='"join":"and",'>
			<cfset nest = '"openParens":"0","closeParens":"1"'><!--- close inner, will apply if there is an other_id_number --->
			<cfset openWith = 0><!--- will apply if there is an other_id_number --->
		</cfif>
		<cfif isDefined("other_id_number") AND len(other_id_number) GT 0>
			<cfif left(other_id_number,1) is "=" OR left(other_id_number,1) is "!">
				...
				<cfset join='"join":"and",'>
			<cfelse>
				<cfset clause = ScriptPrefixedNumberListToJSON(other_id_number, "OTHER_ID_NUMBER", "OTHER_ID_PREFIX", false, openWith, closeWith, "and")>
				<cfset search_json = "#search_json##separator##clause#">
				<cfset separator = ",">
				<cfset join='"join":"and",'>
			</cfif>
		...
		<!--- Second Clause --->
		<!--- openParens+1 for second (type and number) clause --->
		...
		<cfif isDefined("other_id_type_1") AND len(other_id_type_1) GT 0>
			<cfif isDefined("other_id_number_1") AND len(other_id_number_1) GT 0>
				...
				<cfset nest = '"openParens":"1","closeParens":"0"'>
			<cfelse>
				<!--- there is only a type, open and close inner, close outermost )  --->
	...
	<cfelseif has1 >
		...
		<cfif isDefined("other_id_number_1") AND len(other_id_number_1) GT 0>
			<cfif isDefined("other_id_number_1") AND len(other_id_number_1) GT 0 AND isDefined("other_id_type_1") AND len(other_id_type_1) GT 0>
				<!--- has both other_id_type_1 and other_id_number_1 --->
				<cfset nest = '"openParens":"0","closeParens":"1"'>
				<cfset openWith = 0>
				<cfset closeWith = 1>
			<cfelse>
				<!--- has only other_id_number_1 --->
				<cfset nest = '"openParens":"0","closeParens":"0"'>
				<cfset openWith = 0>
				<cfset closeWith = 0>
			</cfif>
	...
	<!--- done with other number, make sure that nest is set back to top level default --->
	<cfset nest = '"openParens":"0","closeParens":"0"'>
	...
	<cfif isDefined("part_attribute_remarks") AND len(part_attribute_remarks) GT 0>
		<cfset field = '"field": "specimen_parts_attribute_remark"'>
		<cfset search_json = search_json & constructJsonForField(join="#join#",field="#field#",value="#part_attribute_remarks#",separator="#separator#",nestDepth="#nest#")>
		<cfset separator = ",">
		<cfset join='"join":"and",'>
	</cfif>
	...
	<cfif isdefined("session.roles") and listcontainsnocase(session.roles,"manage_transactions")>
		...
		<cfif isDefined("permit_type") AND len(permit_type) GT 0>
			<cfset field = '"field": "permit_type"'><!--- in the code table --->
		...
		<cfif isDefined("specific_type") AND len(specific_type) GT 0>
			<cfset field = '"field": "permit_specific_type"'>
...
<cffunction name="saveBasicSrchPref" access="remote">
	...
	<cfif isdefined("session.username") and len(#session.username#) gt 0>
	   <cfthread name="saveBasicSrchThread" >
			...
			<cftransaction>
			<cftry>
				<cfif listFind("IDDetail,TaxaDetail,GeogDetail,CollDetail,SpecDetail,TransactionDetail",id) EQ 0 >
	...
	<cfif isdefined("session.username") and len(#session.username#) gt 0>
	   ...
	   <cfthread name="removeItemsFromResultThread_#tn#" >
			...
			<cftry>
				...
				<cfif variables.grouping_criterion EQ 'family'>
					<cfset bits = ListToArray(variables.grouping_value,"|")>
					<cfset variables.orderBit = bits[1]>
					<cfset variables.familyBit = bits[2]>
				</cfif>
						...
						<cfswitch expression="#variables.grouping_criterion#">
							<cfcase value="parts">
								collection_object_id IN (
									...
									<cfif variables.grouping_value EQ "NULL">
										parts IS NULL
									<cfelse>
										parts = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#variables.grouping_value#">
									</cfif>
								)
							</cfcase>
							...
							<cfcase value="family">
								collection_object_id IN (
									...
									FROM  <cfif ucase(#session.flatTableName#) EQ 'FLAT'>FLAT<cfelse>FILTERED_FLAT</cfif> flat
									WHERE
									<cfif variables.familyBit EQ "NULL">
										family IS NULL
									<cfelse>
										family = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#variables.familyBit#">
									</cfif>
									AND
