						...
						<div class="row">
							<div class="col-12 float-left">
								...
								<div id="identificationDialogList" class="col-12 float-left mt-4 mb-4 px-0">
									<cfset idList = getIdentificationsDetailHTML(collection_object_id = variables.collection_object_id)>
								</div>
							</div>
						</div>
					</div>
...
<!--- Editable Unthreaded: getIdentificationsDetailHTML 
	@param collection_object_id the collection_object_id for the collection object for which to obtain the identifications to edit
	@return html for editing identifications for the specified collection object.
--->
<cffunction name="getIdentificationsDetailHTML" returntype="string" access="remote" returnformat="plain">
	<cfargument name="collection_object_id" type="string" required="yes">
	<cfset variables.collection_object_id = arguments.collection_object_id>
	<cftry>
		<!--- Fetch identifications and any lookup tables you need for dropdowns, e.g. formulas or natures --->
		<cfquery name="ctFormula" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
		...
		<cfquery name="getIdentifications" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
			SELECT identification_id, scientific_name, made_date, nature_of_id, identification_remarks, accepted_id_fg, stored_as_fg, taxa_formula, publication_id, sort_order
			FROM identification
			WHERE collection_object_id = <cfqueryparam cfsqltype="CF_SQL_DECIMAL" value="#variables.collection_object_id#">
			ORDER BY accepted_id_fg DESC, sort_order ASC, made_date DESC
		</cfquery>
		<cfoutput>
			<h2 class="h3">Edit Identifications</h2>
			<form id="editIdentificationsForm" name="editIdentificationsForm">
				<input type="hidden" name="collection_object_id" value="#variables.collection_object_id#">
				<input type="hidden" name="number_of_ids" value="#getIdentifications.recordcount#">
				...
				<cfloop query="getIdentifications" index="i">
					<li style="margin-bottom:2em; border:1px solid #ccc; padding:1em;">
						...
						<div class="form-row">
							...
							<input type="text" name="scientific_name_#i#" id="scientific_name_#i#" class="data-entry-input" value="#encodeForHTML(scientific_name)#" readonly>
						</div>
						<div class="form-row">
							<label for="taxa_formula_#i#" class="data-entry-label">ID Formula:</label>
							<select name="taxa_formula_#i#" id="taxa_formula_#i#" class="data-entry-input" onchange="updateTaxonBVisibilityEdit('#i#');">
								<cfloop query="ctFormula">
									<option value="#ctFormula.taxa_formula#" <cfif taxa_formula EQ ctFormula.taxa_formula>selected</cfif>>#ctFormula.taxa_formula#</option>
								</cfloop>
							</select>
						</div>
						<div class="form-row">
							<label for="made_date_#i#" class="data-entry-label">Date Identified:</label>
							<input type="text" name="made_date_#i#" id="made_date_#i#" class="data-entry-input" value="#made_date#">
						</div>
						<div class="form-row">
							<label for="nature_of_id_#i#" class="data-entry-label">Nature of ID:</label>
							<select name="nature_of_id_#i#" id="nature_of_id_#i#" class="data-entry-input">
								<cfloop query="ctNature">
									<option value="#ctNature.nature_of_id#" <cfif nature_of_id EQ ctNature.nature_of_id>selected</cfif>>#ctNature.nature_of_id#</option>
								</cfloop>
							</select>
						</div>
						<div class="form-row">
							<label for="publication_id_#i#" class="data-entry-label">Publication ID:</label>
							<input type="text" name="publication_id_#i#" id="publication_id_#i#" class="data-entry-input" value="#publication_id#">
						</div>
						<div class="form-row">
						...
						<div class="form-row">
							...
							<select name="accepted_id_fg_#i#" id="accepted_id_fg_#i#" class="data-entry-input">
								<option value="1" <cfif accepted_id_fg EQ 1>selected</cfif>>Yes</option>
								<option value="0" <cfif accepted_id_fg EQ 0>selected</cfif>>No</option>
								<option value="DELETE">Delete</option>
							</select>
						</div>
						<div class="form-row">
							<label for="stored_as_fg_#i#" class="data-entry-label">Stored As:</label>
							<input type="checkbox" name="stored_as_fg_#i#" id="stored_as_fg_#i#" value="1" <cfif stored_as_fg EQ 1>checked</cfif>>
						</div>
						<div class="form-row">
							<label for="sort_order_#i#" class="data-entry-label">Sort Order:</label>
							<input type="text" name="sort_order_#i#" id="sort_order_#i#" class="data-entry-input" value="#sort_order#">
						</div>
						<div class="form-row">
							<input type="button" value="Remove" class="btn btn-xs btn-warning"
								aria-label="Remove this identification"
								onClick="removeIdentification(#identification_id#, #variables.collection_object_id#, reloadIdentificationsDialogAndPage);">
						</div>
					</li>
				</cfloop>
				</ul>
				<div>
					<input type="button" value="Save All Changes" class="btn btn-xs btn-primary" onClick="handleEditIdentifications();">
			...
			<script>
				...
				function handleEditIdentifications() {
					setFeedbackControlState("saveIdentificationsResultsDiv","saving")
					$.ajax({
						...
						success: function (result) {
							if (typeof result.DATA !== 'undefined' && typeof result.DATA.STATUS !== 'undefined' && result.DATA.STATUS[0]=='1') {
								...
								setFeedbackControlState("saveIdentificationsResultsDiv","error")
				...
				function updateTaxonBVisibilityEdit(idx) {
					var formula = document.getElementById('taxa_formula_' + idx).value;
					var taxonBRow = document.getElementById('taxonb_row_' + idx);
					if (taxonBRow) {
						if (formula.includes('B')) {
...
<cffunction name="addIdentification" access="remote" returntype="any" returnformat="json">
	...
	<cfset scientific_name = REReplace(scientific_name, "\bA\b", arguments.taxona, "all")>
	<cfif len(arguments.taxonb)>
		<cfset scientific_name = REReplace(scientific_name, "\bB\b", arguments.taxonb, "all")>
	</cfif>
