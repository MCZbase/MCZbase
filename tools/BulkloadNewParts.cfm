...
<cfset requiredfieldlist = "use_existing,institution_acronym,collection_cde,other_id_type,other_id_number,part_name,preserve_method,lot_count,condition,coll_obj_disposition">
		...
		<cfif #action# is "nothing">
			...
			<cfloop query="getTempTableQC">
				<cfquery name="CollID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					...
					where collection_cde|| '|' ||other_id_type|| '|' ||other_id_number NOT IN (select collection_cde|| '|' ||other_id_type|| '|' ||other_id_number from cataloged_item)
	...
	<!---				<cfquery name="chk" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
		...
		<cfif #action# is "load">
			<cfoutput>
				...
				Parts loaded.
				<a href="/Specimens.cfm?execute=true&builderMaxRows=1&action=builderSearch&nestdepth1=1&field1=COLL_OBJECT%3ACOLL_OBJ_COLLECTION_OBJECT_ID&searchText1=<cfloop query='getTempData'>#getTempData.collection_object_id#<cfif getTempData.collection_object_id gt 1>,<cfelse></cfif></cfloop>" target="_blank">
