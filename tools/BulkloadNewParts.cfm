			...
			<cfif #action# is "validate">
				...
				<cfloop query="getTempTableQC">
					...
					<cfloop index="i" from="1" to="6">
						...
						update cf_temp_parts set
						status = concat(nvl2(status, status || '; ', ''),'Invalid PART_ATT_DETBY. Name matched multiple agent names')
						...
						update cf_temp_parts set 
						concat(nvl2(status, status || '; ', ''),'<span class="font-weight-bold">Invalid PART_ATT_DETBY "' || PART_ATT_DETBY_#i# ||'" does not match </span>')
						...
						update cf_temp_parts set 
						concat(nvl2(status, status || '; ', ''),'<span class="font-weight-bold">Invalid PART_ATT_NAME "' || PART_ATT_NAME_#i# ||'" does not match MCZbase </span>')
