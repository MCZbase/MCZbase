...
<main class="container-fluid px-xl-5 py-3" id="content">
	...
	<cfif #action# is "nothing">
		<cfoutput>
			<p>This tool adds part rows to the specimen record. It creates metadata for part history and includes specimen part attributes fields (that can be empty). The cataloged items must be in the database and they can be entered using the catalog number or other ID. Error messages will appear if the values need to match values in MCZbase. It ignores rows that are exactly the same and alerts you if columns are missing. Additional columns will be ignored. Include column headings, spelled exactly as below. </p>
		...
		<cfoutput>
			...
			<cfloop query="getTempTableQC">
				...
				<cfloop index="i" from="1" to="#NUM_PART_ATTRIBUTE_PAIRS#">
					...
					<cfset partAttCollCde = #findCodeTables.collection_cde#>
				...
				<cfquery name="setPartID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					...
					SET (use_part_id) = (
						...
						from specimen_part, coll_object_remark 
						where
