		...
		<cfoutput>
			<p>This tool adds media records. The media can be related to records that have to be in MCZbase prior to uploading this csv. It ignores rows that are exactly the same and additional columns. Some of the values must appear as they do on the following <a href="https://mczbase.mcz.harvard.edu/vocabularies/ControlledVocabulary.cfm?" class="font-weight-bold">controlled vocabularies</a> lists:
				<ul class="list-group list-group-horizontal">
					<li class="list-group-item"><a href="/vocabularies/ControlledVocabulary.cfm?table=CTMEDIA_LABEL">MEDIA_LABEL (17 values)</a> </li> <span class="mt-1"> | </span>
					<li class="list-group-item"><a href="/vocabularies/ControlledVocabulary.cfm?table=CTMEDIA_RELATIONSHIP">MEDIA_RELATIONSHIP (23 values)</a></li> <span class="mt-1"> | </span>
					<li class="list-group-item"><a href="/vocabularies/ControlledVocabulary.cfm?table=CTMEDIA_TYPE">MEDIA_TYPE (6 values)</a> </li><span class="mt-1"> | </span>
					<li class="list-group-item"><a href="/vocabularies/ControlledVocabulary.cfm?table=CTMIME_TYPE">MIME_TYPE (14 values)</a> </li><span class="mt-1"> | </span>
					<li class="list-group-item"><a href="/vocabularies/ControlledVocabulary.cfm?table=CTMEDIA_LICENSE">MEDIA_LICENSE (7 values)</a></li>
				</ul>
			</p>
		...
		<cfoutput>
			...
			<cfloop query="getTempMedia">
				...
				<cfquery name="mediaLicense" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					...
					WHERE username = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#session.username#">
					and media_license_id in (select media_license_id from CTMEDIA_LICENSE)
					and key = <cfqueryparam cfsqltype="CF_SQL_DECIMAL" value="#getTempMedia.key#"> 
				</cfquery>
						...
						<cfelse>
							...
							<cfelseif table_name is "locality">
								...
								<cfelse>
									<cfquery name="bad" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
										UPDATE cf_temp_media
										SET status = concat(nvl2(status, status || '; ', ''),'locality_id #labelValue# matched #ctLabel.recordcount# records')
										WHERE username = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#session.username#">
										and key = <cfqueryparam cfsqltype="CF_SQL_decimal" value="#getTempMedia.key#">
									</cfquery>
								</cfif>
							...
							<cfelseif table_name is "publication">
								...
								<cfif cPub.recordcount is 1 and len(cPub.publication_id) gt 0>
									<cfquery name="insertMR" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
										insert into 
							...
							<cfelseif table_name is "permit">
								...
								<cfelse>
									<cfset rec_stat=listappend(rec_stat,'permit number #labelValue# matched #cPermit.recordcount# records.',";")>
								</cfif>
