					...
					<cfloop list="#getTempMedia.MEDIA_RELATIONSHIPS#" index="label" delimiters=";">
						...
						<cfelse>
							...
							<cfquery name="getTable" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
								select distinct auto_table from ctmedia_relationship where auto_table = '#LabelValue#'
							</cfquery>
							...
							<cfelseif table_name is "locality">
								...
								<cfelseif cLocality.recordcount is 1 and len(cLocality.locality_id) gt 0 and #idtype# eq 'locality_id'>
									<cfquery name="insRel" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
										insert into cf_temp_media_relations (
											key,
											MEDIA_RELATIONSHIP,
