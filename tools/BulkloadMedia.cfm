				...
				<cfloop query = "getTempMedia2">
					...
					<cfloop index="i" from="1" to="2">
						...
						<cfif ListLen(getMediaRel.related_primary_key) gte #i# >
							...
							<cfelseif #getMediaRel.media_relationship# eq 'shows project' and !isNumeric(getMediaRel.related_primary_key) and len(getMediaRel.related_primary_key) gt 0>
								<cfquery name="chkCOID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
									update cf_temp_media set related_primary_key_#i# =
									(
							...
							<cfelseif #getMediaRel.media_relationship# eq 'shows project' and isNumeric(getMediaRel.related_primary_key) and len(getMediaRel.related_primary_key) gt 0>
								<cfquery name="chkCOID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
									...
									(
										select #theTable#.project_id
										from #theTable#
										where project_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#getMediaRel.related_primary_key#">
									)
									WHERE related_primary_key_#i# is not null AND
										username = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#session.username#"> AND
										key = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#getTempMedia2.key#">
