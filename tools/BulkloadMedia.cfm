				...
				<cfif !isNumeric(created_by_agent_id) gt 0>
					<cfquery name="setAgentID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
						UPDATE
							cf_temp_media
						SET
							created_by_agent_id = (select agent.agent_id from agent,agent_name where agent.agent_id = agent_name.agent_id and agent_name.agent_name = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#getTempMedia.created_by_agent_id#">)
						WHERE
