							...
							<cfelseif getMediaRel.media_relationship contains 'agent' and !isNumeric(getMediaRel.related_primary_key)>
								<cfquery name="chkCOID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
									UPDATE
										cf_temp_media
									SET
										status = concat(nvl2(status, status || '; ', ''),'Agent value not valid')
									WHERE
										related_primary_key_#i# not in (select select agent_name from agent_name where agent_name = '#getMediaRel.related_primary_key#') 
								</cfquery>
