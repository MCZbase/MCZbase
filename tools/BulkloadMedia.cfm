		...
		<cfoutput>
			...
			<cfloop from="1" to="#numberOfVariables#" index="i">
				...
				<cfquery name="checkLabelType" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					...
					WHERE #numberOfVariables# != '#variableValue#' AND
						AND #variablevalue# = '' and #variableName# is not null
						AND username = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#session.username#">
				</cfquery>
			</cfloop>	
			<!---END labels--------------------------------------------->	
				...
				<cfloop query = "getTempMedia2">				
					...
					<cfloop index="i" from="1" to="2">
						...
						<cfif ListLen(getMediaRel.related_primary_key) lte #i# >
							...
							<cfelseif getMediaRel.media_relationship contains 'shows project' and !isNumeric(getMediaRel.related_primary_key)>
								<cfquery name="chkCOID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
									update cf_temp_media set related_primary_key_#i# =
									(
										select project_id
					...
					<cfloop query="viewUpdates">
						...
						<cfif len(#viewUpdates.related_primary_key_2#) eq 0>
							<cfquery name="chkKey" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
								UPDATE
									cf_temp_media
								SET
