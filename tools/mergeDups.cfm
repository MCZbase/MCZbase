...
<a href="mergeDups.cfm?autorun=yep">Autorun</a>
 <p>First Hundred Duplicates:
	<cfquery name="findAllDups" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
		select
...
<cfif #action# is "merge">
	...
	<cfset problems = "">
		...
		<cfif len(#problems#) is 0>
	<cftransaction>
		<cfquery name="goodID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
		...
		<cfloop query="goodID">
			<br>create ID
			<cfquery name="newInsId" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
			...
			<cfquery name="newInsIdTax" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
			...
			<cfquery name="IDAgnt" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
				select * from identification_agent where IDENTIFICATION_ID = #IDENTIFICATION_ID#
			</cfquery>
			<cfloop query="IDAgnt">
			<br>insert ID agent(s)
			<cfquery name="newInsIdAgnt" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
		...
		<cfquery name="badpart" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
			select container.container_id,container.parent_container_id from
				specimen_part,
				coll_object,
				coll_obj_cont_hist,
		...
		<cfquery name="mkenc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
