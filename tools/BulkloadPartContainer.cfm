...
<cfset requiredfieldlist = "OTHER_ID_TYPE,OTHER_ID_NUMBER,COLLECTION_CDE,INSTITUTION_ACRONYM,PART_NAME,PRESERVE_METHOD,CURRENT_REMARKS,NEW_CONTAINER_BARCODE,CONTAINER_BARCODE,PART_COLLECTION_OBJECT_ID">
...
<main class="container-fluid px-xl-5 py-3" id="content">
	...
	<cfif variables.action is "entryPoint">
		<cfoutput>
			<p>&raquo;Use this form to put collection objects (that is, parts) in containers. Only the other_id_type of "catalog number" is accepted in this bulkloader. The unique string representing the container is used (not the container_id consisting of only numbers&mdash;the ID will be generated and visible in the validation step). Parts and containers must already exist.</p>
			<p><em>Multiple Parts OK:</em> This bulkloader can be used for specimen records with multiple parts as long as the combination of the following column values are unique: part name, preserve method, and part remarks.</p>
			<p>&raquo;Upload a comma-delimited text file (csv). You can either enter the data using the template below or edit a part report. </p>
			<p><em>Edit a Part Report:</em> The best way to avoid ambiguous parts is to use a part report from the specimen search results > Manage > Part Download/Report feature. When using the part report, select the "Download Parts CSV with Container placements", check that the parts downloaded are expected, and add the new_container_barcode (a.k.a., new unique_container_id). The additional columns downloaded will be ignored and will appear in the warning section of the validation screen with any other columns not needed for the bulkload. </p>
