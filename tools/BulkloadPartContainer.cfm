...
<main class="container-fluid px-xl-5 py-3" id="content">
	...
	<cfif variables.action is "entryPoint">
		<cfoutput>
			<p>Use this form to put collection objects (that is, parts) in containers. Only other_id_type of "catalog number" is accepted in this bulkloader. The unique string representing the container is used (not the container_id consisting of only numbers--it will be generated and visible in the validation step). Parts and containers must already exist.</p>
