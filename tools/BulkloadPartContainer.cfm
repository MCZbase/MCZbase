...
<main class="container-fluid px-xl-5 py-3" id="content">
	...
	<cfif variables.action is "entryPoint">
		<cfoutput>
			<p>Use this form to put collection objects (that is, parts) in containers. Only the other_id_type of "catalog number" is accepted in this bulkloader. The unique string representing the container is used (Container Unique Identifier). Parts and containers must already exist.</p>
			<p><em>Multiple Parts OK:</em> This bulkloader can be used for specimen records with multiple parts as long as the combination of the following column values are unique: part_name, preserve_method, and part_remarks. The part_collection_object_id will be unique (but temporary) when autogenerated from part_name, preserve_method, and part_remarks.</p>
			<p>Upload a comma-delimited text file (csv). You can either enter the data using the template below or edit a part report. </p>
			<p><em>Edit a Part Report:</em> The best way to avoid ambiguous parts is to use a part report from the specimen search results > Manage > Part Download/Report feature. When using the part report, select the "Download Parts CSV with Container placements", check that the parts downloaded are expected, and add values to the column NEW_CONTAINER_BARCODE to hold the container barcode (a.k.a., unique_container_id) of the container to place the part into. The additional columns downloaded will be ignored and will appear in the warning section of the validation screen with any other columns not needed for the bulkload. </p>
...
<cfif variables.action is "validate">
	...
	<cfoutput>
		...
		<cfloop query="dataParts">
			...
			<!---Get the part_collection_object_id based on the specimen record's collection_object_id only if part_collection_object_id is empty--->
		...
		<cfloop query="getTempTableQC1">
			...
			<cfif len(part_collection_object_id) gt 0>
				<cfquery name="getPartCollID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					UPDATE cf_temp_barcode_parts
					SET status = concat(nvl2(status, status || '; ', ''), 'PART_COLLECTION_OBJECT_ID not valid (and catalog info. not valid?)')
				...
				<cfquery name="probPreserveMethod" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					UPDATE cf_temp_barcode_parts
					SET
						status = concat(nvl2(status, status || '; ', ''),'preserve_method is invalid')
					where 
						preserve_method not in (select preserve_method from specimen_part where collection_object_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#getTempTableQC1.part_collection_object_id#">)
						AND username = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#session.username#">
						AND key = <cfqueryparam cfsqltype="CF_SQL_DECIMAL" value="#getTempTableQC1.key#">
				</cfquery>
