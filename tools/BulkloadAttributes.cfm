...
<cftry>
		...
		<cfset headersRecord = headersRecord.get(JavaCast("int",#i#))>
		...
		<cfif headersRecord NEQ "">
			...
			<cfloop index="i" from="0" to="#headersRecord.size() - 1#">
				...
				<cfif i LTE arrayLen(expectedHeaders)>
					<cfif headersRecord NEQ expectedHeaders[i]>
						<cfoutput>[#i#] #headersRecord# is not found in the list of expected headers.<br></cfoutput>
					</cfif>
				<cfelse>
					<cfoutput>Additional header #headersRecord# found in the CSV file.<br></cfoutput>
				</cfif>
