		...
		<cfoutput>
			...
			<cfif getType.attribute is 'Associated MCZ Collection'>
				<cfquery name="act4" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#">
					...
					WHERE attribute_value not in (select collection.collection from ctcollections_full_names,collection where collection.collection = ctcollections_full_names.collection_name and collection.collection_cde = cf_temp_attributes.collection_cde)
						AND username = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#session.username#">
				</cfquery>
			</cfif>
			<cfif getType.attribute is 'age class'>
