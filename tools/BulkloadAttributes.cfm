...
<main class="container py-3" id="content">
	...
	<cfif #action# is "nothing">
		<cfoutput>
			<p>This tool adds attributes to the specimen record. The attribute has to be in the code table prior to uploading this .csv. It ignores rows that are exactly the same. The attributes and attribute values must appear as they do on the <a href="https://mczbase.mcz.harvard.edu/vocabularies/ControlledVocabulary.cfm?" class="font-weight-bold">controlled vocabularies</a> list. You will want to check the to see if the ATTRIBUTE_TYPE is in the <a href="https://mczbase.mcz.harvard.edu/vocabularies/ControlledVocabulary.cfm?table=CTATTRIBUTE_CODE_TABLES">ATTRIBUTE_CODE_TABLES</a> and then see if the ATTRIBUTE_VALUE has a table of its own. For example, if you are in entomology and want to add "sex=male" as an attribute to a specimen record, check the ATTRIBUTE_TYPE table to see if "sex" is listed next to "Ent"  and then go to the ATTRIBUTE_CODE_TABLES to see if there are specific values to choose. I see for sex, it says ctsex_cde. Next, I look for SEX_CDE on the ATTRIBUTES_CODE_TABLES page and click on it to find the format of "male" that the Ent dept prefers. In this case it is "Male". Remember to check capitalization and spelling.</p>
	...
	<cfif #action# is "validate">
		...
		<cfoutput>
			<cfquery name="getType" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#" result="updateAttributes_result">
			...
			<cfquery name="miap" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#">
				UPDATE cf_temp_attributes
				SET status = 'attribute_not_found; for invalid identifier error check case and spelling'
