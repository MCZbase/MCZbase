...
<cfif action is "makeTemplate">
	<cfset header="DIVISION,PHYLCLASS,SUBCLASS,INFRACLASS,SUPERORDER,PHYLORDER,SUBORDER,INFRAORDER,SUBSECTION,SUPERFAMILY,FAMILY,SUBFAMILY,GENUS,SUBGENUS,SPECIES,SUBSPECIES,VALID_CATALOG_TERM_FG,SOURCE_AUTHORITY,AUTHOR_TEXT,TRIBE,INFRASPECIFIC_RANK,TAXON_REMARKS,PHYLUM,KINGDOM,NOMENCLATURAL_CODE,INFRASPECIFIC_AUTHOR,TAXON_STATUS">
...
<cfoutput>
	...
	<cffile action="READ" file="#FiletoUpload#" variable="fileContent" charset="windows-1252">
...
<cfoutput>
	<cfquery name="reset" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
		update cf_temp_taxonomy set status = null
	</cfquery>

	<cfquery name="bad2" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
...
<cfoutput>
	...
	<p>
	<cfset colList = "DIVISION,PHYLCLASS,SUBCLASS,INFRACLASS,SUPERORDER,PHYLORDER,SUBORDER,INFRAORDER,SUBSECTION,SUPERFAMILY,FAMILY,SUBFAMILY,GENUS,SUBGENUS,SPECIES,SUBSPECIES,VALID_CATALOG_TERM_FG,SOURCE_AUTHORITY,AUTHOR_TEXT,TRIBE,INFRASPECIFIC_RANK,TAXON_REMARKS,PHYLUM,KINGDOM,NOMENCLATURAL_CODE,INFRASPECIFIC_AUTHOR,TAXON_STATUS">
	...
	<cfoutput>
		<cftransaction>
			<cfloop from ="1" to="#numberOfRecords#" index="i">
				...
				<cfset SUBCLASS = evaluate("SUBCLASS_" & key)>
				<cfset SUPERFAMILY = evaluate("SUPERFAMILY_" & key)>
				<cfset TAXON_STATUS = evaluate("TAXON_STATUS_" & key)>
				...
				<cfquery name="edTaxa" datasource="user_login" username='#session.username#' password="#decrypt(session.epw,cookie.cfid)#">
					#preserveSingleQuotes(sql)#
				</cfquery>
				<cfquery name="killTemp" datasource="user_login" username='#session.username#' password="#decrypt(session.epw,cookie.cfid)#">
...
<cfoutput>
	<cfquery name="data" datasource="user_login" username='#session.username#' password="#decrypt(session.epw,cookie.cfid)#">
		select * from cf_temp_taxonomy
	</cfquery>
	<cftransaction>
	<cfloop query="data">
		<cfquery name="nid" datasource="user_login" username='#session.username#' password="#decrypt(session.epw,cookie.cfid)#">
			select sq_taxon_name_id.nextval nid from dual
		</cfquery>
		<cfquery name="newTaxa" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
