				...
				<cfloop list="#fieldList#" item="aField">
					<cfif ListContainsNoCase(requiredFieldList,aField)>
						...
						<cfif NOT ListContainsNoCase(foundHeaders,aField)>
							<cfset errorMessage = "#errorMessage# <i class='fas fa-arrow-right text-info'></i><strong class='text-danger'> &nbsp;#aField#<br></strong>">
						</cfif>
					</cfif>
				</cfloop>
						...
						<cfif NOT ListLen(ListRemoveDuplicates(foundHeaders)) EQ ListLen(foundHeaders)>
							...
							<cfloop list="#foundHeaders#" item="aField">
								<cfif listValueCount(foundHeaders,aField) GT 1>
										<li class="pt-1 px-4"><i class='fas fa-arrow-right text-info'></i> <strong class="text-info">#i# #aField#</strong> </1i>
								</cfif>
							<cfset i=i+1>
							</cfloop>
						</cfif>
