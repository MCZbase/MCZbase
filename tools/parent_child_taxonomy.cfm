...
<cfif variables.shouldQuery>
	<cfquery name="relationshipPairs" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
		...
		SELECT
			...
			sourcePAN.agent_name AS source_determiner,
			...
			relatedPAN.agent_name AS related_determiner,
			...
			/* subtract one to exclude the displayed accepted identification from "other identifications" metadata */
