			...
			<cfoutput>
				...
				<cftry>
					...
					<h3 class="h3">
						...
						<cfelse>
							Successfully read #loadedRows# records from the CSV file.  Next <a href="/tools/BulkloadEditedParts.cfm?action=validate">click to validate</a>.
						</cfif>
					</h3>
					<cfcatch>
						<h3 class="h4">
		...
		<cfif #action# is "validate">
			...
			<cfloop query="getTempTableQC">
				...
				<cfquery name="bads" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					update cf_temp_parts set concat(nvl2(status, status || '; ', ''),'Invalid part_name')
				...
				<cfquery name="bads" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					update cf_temp_parts set concat(nvl2(status, status || '; ', ''),'Invalid preserve_method')
				...
				<cfquery name="bads" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					update cf_temp_parts set concat(nvl2(status, status || '; ', ''),'Invalid new_preserve_method')
				...
				<cfquery name="isValid" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					update cf_temp_parts set concat(nvl2(status, status || '; ', ''),'Invalid use_existing flag')
						where use_existing not in ('0','1') OR
						use_existing is null
				</cfquery>
				<cfquery name="bads" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					update cf_temp_parts set concat(nvl2(status, status || '; ', ''),'Invalid CONTAINER_UNIQUE_ID')
				...
				<cfquery name="bads" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					update cf_temp_parts set concat(nvl2(status, status || '; ', ''),'Invalid DISPOSITION')
				...
				<cfquery name="bads" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					update cf_temp_parts set concat(nvl2(status, status || '; ', ''),'Invalid CONTAINER_TYPE')
				...
				<cfquery name="bads" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					update cf_temp_parts set concat(nvl2(status, status || '; ', ''),'Invalid CONDITION')
					where CONDITION is null
				</cfquery>
				<cfquery name="bads" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					update cf_temp_parts set concat(nvl2(status, status || '; ', ''),'invalid lot_count_modifier')
					where lot_count_modifier NOT IN (
						select modifier from ctnumeric_modifiers
						)
				</cfquery>
				<cfquery name="bads" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					update cf_temp_parts set concat(nvl2(status, status || '; ', ''),'Invalid LOT_COUNT')
					where (
				...
				<cfquery name="bads" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					update cf_temp_parts set concat(nvl2(status, status || '; ', ''),'Invalid CHANGED_DATE')
...
<!---				<cfquery name="chk" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
			...
			<cfif #allValid.cnt# is 0>
				<h3 class="mt-2"><span class="text-success">Validation checks passed</span>. Look over the table below and <a href="BulkloadNewParts.cfm?action=load">click to continue</a> if it all looks good. Or, <a href="/tools/BulkloadNewParts.cfm">Start over</a>.
				</h3>
			<cfelse>
				<h3 class="mt-2">You must fix everything above to proceed. <a href="/tools/BulkloadNewParts.cfm">Try again.</a></h3>
			</cfif>
