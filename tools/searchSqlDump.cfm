...
<!--- Function normalizeSql collapses runs of whitespace so that a reformatting change in
	SearchSql.cfm does not register as a behaviour change when baselines are diffed.
...
--->
<cffunction name="normalizeSql" access="private" returntype="string" output="false">
...
<cfoutput>
--- basJoin ---
#normalizeSql(basJoin)#

--- basWhere ---
#normalizeSql(basWhere)#

--- basOrder ---
#normalizeSql(basOrder)#
</cfoutput>
...
--- whereClauses (array contract, post phase 3): #arrayLen(variables.whereClauses)# clause(s) ---
	...
	<cfloop from="1" to="#arrayLen(variables.whereClauses)#" index="variables.i">
		<cfoutput>[#variables.i#] #normalizeSql(variables.whereClauses[variables.i])#
</cfoutput>
	</cfloop>
</cfif>
