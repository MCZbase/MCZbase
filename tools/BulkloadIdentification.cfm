		...
		<cfoutput>
			...
			<cfquery name="isSci" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#">
				UPDATE cf_temp_ID SET scientific_name= 
					(SELECT scientific_name FROM taxonomy WHERE scientific_name = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#isTaxa1.TaxonomyTaxonName#">)
				WHERE username = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#session.username#">
			</cfquery>
			...
			<cfquery name="getTID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#">
				update cf_temp_ID set scientific_name =
				(select scientific_name from taxonomy where taxonomy.scientific_name = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#isTaxa1.TaxonomyTaxonName#">)
				WHERE username = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#session.username#">
			</cfquery>
