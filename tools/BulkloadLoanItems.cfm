	...
	<cfif #action# is "load">
		<h2 class="h4">Third step: Apply changes.</h2>
			<cfoutput>
				...
				<cfquery name="getCountsParts" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
					SELECT count(distinct PARTID) ctObj 
					FROM cf_temp_loan_item
					WHERE username = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#session.username#">
				</cfquery>
				<cfquery name="getCountsLoan" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
				...
				<cftry>
					...
					<cfif getTempData.recordcount eq loan_updates>
						<p>Number of loan items updated: #getCountsParts.ctObj# in #loan_updates# cataloged_items (#getCountsLoan.cttrans# loans)</p>
