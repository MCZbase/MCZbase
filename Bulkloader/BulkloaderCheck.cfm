...
<cfset loadedMsg = "">
			...
			<cfif len(#catalog_number#) gt 0>
				<cfif not isnumeric(#catalog_number#)>
					<cfset loadedMsg = "#loadedMsg#; ::catalog_number:: must be numeric.">
				</cfif>
				<cfif #catalog_number# is 0>
					<cfset loadedMsg = "#loadedMsg#; ::catalog_number:: may not be 0. Did you mean NULL?">
				</cfif>
				...
				<cfquery name="isValidCatNum" datasource="#mcat#">
					...
					<cfif len(#catalog_number_suffix#) gt 0>
						and catalog_number_suffix = '#catalog_number_suffix#'
					</cfif>
				</cfquery>
				<cfif #isValidCatNum.recordcount# is 0>
					<cfset catnum = #catalog_number#>
					<cfset catnumprefix=#catalog_number_prefix#>
					<cfset catnumsuffix=#catalog_number_suffix#>
				<cfelse>
	...
	<cfif len(#thisPartName#) gt 0>
		...
		<cfif len(#thisPartBarCode#) GT 0>
			...
			<cfif #getID.recordcount# neq 1>
				<cfset loadedMsg = "#loadedMsg#; ::part_container_unique_id_#i#:: (#thisPartBarCode#) was not found. Container unique IDs must match pre-existing containers">
			</cfif>
			<cfif len (#thisPartContainerLabel#) is 0>
				<cfset loadedMsg = "#loadedMsg#; ::container_name_#i#:: is required when loading container_unique_ids.">
			</cfif>
		<cfelseif len (#thisPartContainerLabel#) gt 0>
			<!---- label but no barcode ---->
			<cfset loadedMsg = "#loadedMsg#; You must supply a unique id for ::container_name_#i#::">
		</cfif>
	</cfif>
</cfloop>
		<cfif len(#accn#) is 0>
