...
<!--- Get the card-body HTML for in-context project annotation display.
  Renders root-level annotations on the given project record as a list-group with conversation replies,
  or a "no annotations" message when none exist.  Respects session visibility: coldfusion_user role
  sees all annotations; other users see only unmasked annotations or their own.
  Returns only the div.card-body HTML; callers are responsible for the card heading (with count and
  Annotate/Edit Annotations button).  access="remote" allows callers to reload the card body via AJAX
  after annotations are created or updated.
  @param project_id the project.project_id whose annotations are to be rendered.
  @return string div.card-body HTML ready for output.
--->
<cffunction name="getProjectAnnotationCardBodyHtml" returntype="string" access="remote" returnformat="plain">
	...
	<cftry>
		<cfquery name="annQuery" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#" timeout="#Application.short_timeout#">
			...
			WHERE
				...
				AND annotations.target_primary_key = <cfqueryparam cfsqltype="CF_SQL_DECIMAL" value="#arguments.project_id#">
				<cfif NOT listcontainsnocase(session.roles, "coldfusion_user")>
					AND (annotations.mask_annotation_fg = 0 OR annotations.cf_username = <cfqueryparam value="#session.username#" cfsqltype="CF_SQL_VARCHAR">)
				</cfif>
			ORDER BY annotations.annotate_date
	...
	<cfcatch>
		<cfset cfcatchToErrorMessage(cfcatch)>
		<cfset reportError(functionName="getProjectAnnotationCardBodyHtml", note="project_id=#arguments.project_id#")>
		<cfheader statuscode="500" statustext="Internal Server Error">
		<cfset cardBodyHtml = '<div class="card-body py-2"><p class="my-2 text-danger small">Error loading annotations.</p></div>'><!--- ' --->
		<cfabort>
	</cfcatch>
...
<!--- Get the card-body HTML for in-context publication annotation display.
  Renders root-level annotations on the given publication record as a list-group with conversation
  replies, or a "no annotations" message when none exist.  Respects session visibility:
  coldfusion_user role sees all annotations; other users see only unmasked annotations or their own.
  Returns only the div.card-body HTML; callers are responsible for the card heading (with count and
  Annotate/Edit Annotations button).  access="remote" allows callers to reload the card body via AJAX
  after annotations are created or updated.
  @param publication_id the publication.publication_id whose annotations are to be rendered.
  @return string div.card-body HTML ready for output.
--->
<cffunction name="getPublicationAnnotationCardBodyHtml" returntype="string" access="remote" returnformat="plain">
	...
	<cftry>
		<cfquery name="annQuery" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#" timeout="#Application.short_timeout#">
			...
			WHERE
				...
				AND annotations.target_primary_key = <cfqueryparam cfsqltype="CF_SQL_DECIMAL" value="#arguments.publication_id#">
				<cfif NOT listcontainsnocase(session.roles, "coldfusion_user")>
					AND (annotations.mask_annotation_fg = 0 OR annotations.cf_username = <cfqueryparam value="#session.username#" cfsqltype="CF_SQL_VARCHAR">)
				</cfif>
			ORDER BY annotations.annotate_date
