...
<cffunction name="getAnnotationSearchFilters" returntype="struct" access="public">
	...
	<cfquery name="families" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
		...
		WHERE upper(annotations.target_table) = 'TAXON_NAME'
			AND taxonomy.family IS NOT NULL
...
<cffunction name="findAnnotations" returntype="query" access="public">
	...
	<cfquery name="annotationResults" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
		SELECT
			...
			CASE
				WHEN upper(annotations.target_table) = 'COLLECTION_OBJECT' THEN to_char(annotations.collection_object_id)
				WHEN upper(annotations.target_table) = 'TAXON_NAME' THEN to_char(annotations.taxon_name_id)
				WHEN upper(annotations.target_table) = 'PUBLICATION' THEN to_char(annotations.publication_id)
				WHEN upper(annotations.target_table) = 'PROJECT' THEN to_char(annotations.project_id)
				ELSE to_char(annotations.target_primary_key)
			END target_key
		FROM
			annotations
			LEFT OUTER JOIN cataloged_item ON annotations.collection_object_id = cataloged_item.collection_object_id
			...
			LEFT OUTER JOIN taxonomy ON annotations.taxon_name_id = taxonomy.taxon_name_id
			LEFT OUTER JOIN publication ON annotations.publication_id = publication.publication_id
			LEFT OUTER JOIN project ON annotations.project_id = project.project_id
		...
		WHERE
			...
			<cfif len(targetTableFilter) GT 0>
				AND upper(annotations.target_table) = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#targetTableFilter#">
			<cfelse>
				AND upper(annotations.target_table) IN ('COLLECTION_OBJECT','TAXON_NAME','PUBLICATION','PROJECT')
			</cfif>
			...
			<cfif len(trim(arguments.collection_object_id)) GT 0 AND isNumeric(arguments.collection_object_id)>
				AND annotations.collection_object_id = <cfqueryparam cfsqltype="CF_SQL_DECIMAL" value="#trim(arguments.collection_object_id)#">
			</cfif>
			...
			<cfif len(trim(arguments.taxon_name_id)) GT 0 AND isNumeric(arguments.taxon_name_id)>
				AND annotations.taxon_name_id = <cfqueryparam cfsqltype="CF_SQL_DECIMAL" value="#trim(arguments.taxon_name_id)#">
			</cfif>
			...
			<cfif len(trim(arguments.publication_id)) GT 0 AND isNumeric(arguments.publication_id)>
				AND annotations.publication_id = <cfqueryparam cfsqltype="CF_SQL_DECIMAL" value="#trim(arguments.publication_id)#">
			</cfif>
			<cfif len(trim(arguments.project_id)) GT 0 AND isNumeric(arguments.project_id)>
				AND annotations.project_id = <cfqueryparam cfsqltype="CF_SQL_DECIMAL" value="#trim(arguments.project_id)#">
			</cfif>
		ORDER BY
			upper(annotations.target_table),
			target_key,
