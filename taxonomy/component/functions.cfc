	...
	<cftry>
		...
		<cfif tax_pub.recordcount gt 0>
			<cfloop query="tax_pub">
				<cfset result=result & "<div class='col-12 my-2 px-1'><ul>">
				<!--- Create a link out of author year. in the publication, ensure that link closes. --->
				<cfset publication = "<li><a href='SpecimenUsage.cfm?publication_id=#publication_id#' target='_blank'><img src='/images/noThumbGray.jpg' height='40' width='28' alt='thumbnail icon for document' class='mr-2'>" & rereplace(formatted_publication,'([0-9]\.)','\1</a>') >
					<cfif NOT findNoCase('</a>',publication)><cfset publication = publication & "</a></li>"</cfif>
