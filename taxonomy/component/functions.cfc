...
<cffunction name="getTaxonPublicationsHtml" returntype="string" access="remote" returnformat="plain">
	...
	<cftry>
		...
		<cfif tax_pub.recordcount gt 0>
			<cfset result=result & "<div class='col-12 my-2 px-0'><ul class='list-unstyled'>">
			<cfloop query="tax_pub">
				<!--- Create a link out of author year. in the publication, ensure that link closes. --->
				<cfset publication = "<li><a href='SpecimenUsage.cfm?publication_id=#publication_id#' target='_blank'><img src='/images/noThumbGray.jpg' width='28' height='40' alt='document icon' class='mr-2'>" & rereplace(formatted_publication,'([0-9]\.)','\1</a><span class='d-block') >
					<cfif NOT findNoCase('</a>',publication)><cfset publication = publication & "</a>"></cfif>
				<cfset result=result & "#publication#</span></li>">
