	...
	<cftry>
		...
		<cfif tax_pub.recordcount gt 0>
			...
			<cfloop query="tax_pub">
				...
				<cfset result=result & "">
					<cfset result=result & "</li><li class='mb-1 list-group-item col-1 float-right py-1'><button class='btn-xs btn-warning mx-1 col-1 float-left' onclick=' confirmDialog("" Remove Relatioship?"",""Remove?"", function() { removeTaxonPub(#taxonomy_publication_id#); } );' value='Remove' title='Remove' aria-label='Remove this Publication from Taxonomy'>Remove</button></ul>">
				</cfloop>
