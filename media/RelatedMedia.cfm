...
<cfoutput>
	...
	<!---	and ct.description <> 'ledger'--->
		...
		and ct.auto_table = 'cataloged_item'
		UNION
		select ce.collecting_event_id as pk, ct.media_relationship as rel, ct.auto_table as wlabel, ct.label as label, ct.auto_table
		...
		UNION
		select loan.transaction_id as pk, ct.media_relationship as rel, ct.auto_table as wlabel, ct.label as label, ct.auto_table
		from loan
		...
		UNION
		select accn.transaction_id as pk, ct.media_relationship as rel, ct.auto_table as wlabel, ct.label as label, ct.auto_table
		from accn
		...
		UNION
		select locality.locality_id as pk, ct.media_relationship as rel, ct.auto_table as wlabel, ct.label as label, ct.auto_table
		from locality
		...
		UNION
		select agent.agent_id as pk, ct.media_relationship as rel, an.agent_name as wlabel, ct.label as label, ct.auto_table
		from agent_name an
																	...
																	<ul class="list-group px-0">
																		<li class="list-group-item px-0 mx-1">
																			...
																			<div class="#activeimg# image#i#" id="mediaBlock#relm.media_id#" style="height:220px;">
																				<div class="px-0">
																					<span class="px-2 d-block mt-1 small90 font-weight-lessbold text-center"><cfif media_rel.media_relationship eq 'ledger entry for cataloged_item'>Ledger Entry for Cataloged Item<cfelse>#spec.label#</cfif>  <br>(media/#relm.media_id#)
																					</span> 
																					#mediablock#
																				</div>
																			</div>
						...
						<cfelse>
							...
							<!---<cfif media.recordcount gt 0>
								<cfquery name="relmer" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#">
									...
									left join mczbase.ctmedia_relationship ct on mr.media_relationship = ct.media_relationship
									where c.collection_object_id =<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#collid.pk#">
									and (ct.description = 'publication' OR ct.description = 'ledger')
									and m.auto_host <> 'nrs.harvard.edu'
									UNION
