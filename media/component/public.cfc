											...
											<cfif media_rel.media_relationship contains 'container'>
												<cfif oneofus eq 1>
													<cfloop query="getContainers">
														<a class="font-weight-lessbold" href="/containers/Containers.cfm?search_term=#encodeForUrl('=' & getContainers.label)#&execute=true"> #encodeForHtml(getContainers.label)# (#encodeForHtml(getContainers.container_type)#)</a><cfif getContainers.recordcount gt 1><span>, </span></cfif>
													</cfloop>
												<cfelse>
													<span class="d-inline font-italic">Hidden</span>
												</cfif>
				...
				<cfloop query="media">
					...
					<table class="table table-responsive-sm mb-3 border-none small90">
						...
						<tbody>
							...
							<cfif listcontainsnocase(session.roles,"manage_media")>
								<tr class="border mt-2 p-2">
									<th scope="row">Media URI </th><td>#media.media_uri#</td>
								</tr>
							</cfif>
							<cfif listcontainsnocase(session.roles,"manage_media")>
