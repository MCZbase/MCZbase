								...
								<cfloop query="media_rel"><span class="text-capitalize">#media_rel.label#</span>
									<div class="comma2 d-inline">
										...
										<cfif media_rel.media_relationship contains 'shows agent'>:<cfloop query="agents">
											<cfquery name="relm2" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#">select agent_name.agent_id,media_relations.media_id, agent_name.agent_name from media_relations left join agent_name on agent_name.agent_id = media_relations.related_primary_key where media_relations.media_relationship='shows agent' and agent_name.agent_name = '#agents.agent_name#'</cfquery><a class="font-weight-lessbold" href="/agents/Agent.cfm?agent_id=#relm2.agent_id#">#relm2.agent_name# (#relm2.media_id#)</a><span>, </span></cfloop>
										</cfif>
