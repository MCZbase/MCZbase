							...
							<td>
								<cfloop query="media_rel"><span class="text-capitalize">#media_rel.label##plural#</span>
									<div class="comma2 d-inline">
										...
										<cfloop query="publication"><cfquery name="relm7" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#">select distinct p.publication_title,fp.formatted_publication as pub_long from publication p, formatted_publication fp where p.publication_id = fp.publication_id and p.publication_id = <cfqueryparam value=#spec.pk# CFSQLType="CF_SQL_VARCHAR"> and fp.format_style = 'long' </cfquery> #relm7.pub_long# <span> &mdash; </span></cfloop>
										</cfif>
										<cfif media_rel.media_relationship eq 'cataloged_item'>: 
