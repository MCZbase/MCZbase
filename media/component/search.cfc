							...
							<td>
								<cfloop query="media_rel"><span class="text-capitalize">#media_rel.label#</span>
									<div class="comma2 d-inline">
										...
										<cfif media_rel.media_relationship contains 'shows agent'>:<cfloop query="agents"><cfquery name="relm2" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#"> select m.media_id,an.agent_id from agent_name an, media_relations m where an.agent_name=#agents.agent_name# and an.agent_id=m.related_primary_key and m.media_relationship='shows agent'</cfquery><a class="font-weight-lessbold" href="/agents/Agent.cfm?agent_id=#rel2.agent_id#"> #agents.agent_name# </a><span>, </span></cfloop>
										</cfif>
