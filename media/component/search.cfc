	...
	<cftry>
		...
		<cfloop query="media">
			...
			<cfquery name="media_rel" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#">
				select distinct mr.media_relationship as pk, ct.description,ct.Label as label,  ct.auto_table 
				from media_relations mr 
				left join mczbase.ctmedia_relationship ct on mr.media_relationship = ct.media_relationship
				where mr.media_id = <cfqueryparam cfsqltype="CF_SQL_DECIMAL" value="#media.media_id#">
				UNION
				select distinct mr.media_relationship as pk,flat.cat_num,ct.Label as label, ct.auto_table
				from flat
				...
				UNION
				select distinct mr.media_relationship as pk,p.publication_title,ct.Label as label, ct.auto_table
				from publication p
				left join media_relations mr on mr.RELATED_PRIMARY_KEY = p.publication_id 
				left join media m on m.media_id = mr.media_id
				left join citation c on c.publication_id = p.publication_id
