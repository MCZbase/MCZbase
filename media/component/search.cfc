					...
					<cfelse>
						...
						<cfif #captionAs# EQ "textCaptionLong"><!---This is for use when a caption of 197 characters is needed --->
							<cfif len(showTitleText) GT 232>
								<cfset showTitleText = "#left(showTitleText,213)#..." >
							</cfif>
						</cfif>
						...
						<tr>
							...
							<cfloop query="media_rel"><span class="text-capitalize">#media_rel.label#</span>
								<div class="comma2 d-inline">
									...
									<cfloop query="publication"><cfquery name="relm7" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#">select distinct p.publication_title,fp.formatted_publication as pub_long, m.media_uri from publication p, formatted_publication fp, media_relations mr,media m where mr.related_primary_key = p.publication_id and mr.media_id = m.media_id and p.publication_id = fp.publication_id and p.publication_id = <cfqueryparam value=#publication.pk# CFSQLType="CF_SQL_VARCHAR"> and mr.media_relationship = 'publication' and fp.format_style = 'long' </cfquery><a class="font-weight-lessbold" href="#relm7.media_uri#">#relm7.pub_long# </a><span> &##8226;&##8226; </span> </cfloop>
									</cfif>
