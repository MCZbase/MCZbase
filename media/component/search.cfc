						...
						<tr>
							...
							<cfloop query="media_rel"><span class="text-capitalize">#media_rel.label##plural#</span>
								<div class="comma2 d-inline">
									...
									<cfif media_rel.media_relationship eq 'documents accn'>:<cfloop query="accns"><cfquery name="relm5" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#"> select m.media_id,ac.transaction_id from accn ac left join media_relations m on ac.transaction_id=m.related_primary_key left join flat on ac.transaction_id = flat.accn_id where m.media_relationship = 'documents accn' and ac.transaction_id=<cfqueryparam cfsqltype="cf_sql_varchar" value="#accns.transaction_id#" /> </cfquery> #relm5.transaction_id#<span>, </span></cfloop>
									</cfif>
