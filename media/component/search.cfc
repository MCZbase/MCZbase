							...
							<td>
								<cfloop query="media_rel"><span class="text-capitalize">#media_rel.label#</span>
									<div class="comma2 d-inline">
										...
										<cfif media_rel.media_relationship contains 'shows agent'>:<cfloop query="agents"><cfquery name="relm2" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#"> select m.media_id,an.agent_id from agent_name an, media_relations m where agent_name=<cfqueryparam value=#agents.agent_name# CFSQLType="CF_SQL_DECIMAL" > and an.agent_id=m.related_primary_key and m.media_relationship='shows agent'</cfquery><a class="font-weight-lessbold" href="/agents/Agent.cfm?agent_id=#relm2.agent_id#"> #agents.agent_name#</a><span>: </span><cfloop query="relm2"><a class="font-weight-lessbold" href="/media/#relm2.media_id#"> media/#relm2.media_id#</a><span>, </span></cfloop></cfloop>
										</cfif>
