							...
							<td>
								<cfloop query="media_rel"><span class="text-capitalize">#media_rel.label#</span>
									<div class="comma2 d-inline">
										...
										<cfif media_rel.media_relationship eq 'documents accn'>:<cfloop query="accns"><cfquery name="relm5" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#"> select m.media_id,accn.transaction_id from accn ac left join media_relations m on ac.accn_num=m.related_primary_key where related_primary_key=<cfqueryparam cfsqltype="cf_sql_decimal" value="#accn.transaction_id#" /> and m.media_relationship = 'documents accn'</cfquery> Accn: #relm5.transaction_id#<span>, </span></cfloop>
										</cfif>
