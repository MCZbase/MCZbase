<cfinclude template="/includes/_header.cfm">
	<cfoutput>
		<cffunction name="getRec">
			...
			<cfquery name="rec" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
				select
			...
			<cfif len(rec.media) gt 0>
				<cfquery name="tn" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
		...
		<cfset limit=20>
		<cfquery name="dupRec" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cookie.cfid)#">
			select * from (	
				select 
					count(*) cnt,
					display_value 
