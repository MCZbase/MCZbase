			...
			<cfelseif lookupUUID.recordcount GT 0>
				<cfif lookupUUID.disposition IS "exists" AND lookupUUID.target_table IS "coll_object" AND lookupUUID.guid_is_a IS "occurrenceID">
					...
					<cfquery name="getCatItem" datasource="cf_dbuser" timeout="#Application.short_timeout#">
						SELECT co.coll_object_type, coll.institution_code, coll.collection_code, ci.catalog_number
						FROM coll_object co
						left join specimen_part on colo.collection_object_id = specimen_part.collection_object_id
						...
						WHERE co.collection_object_id = <cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#lookupUUID.co_collection_object_id#">
					</cfquery>
