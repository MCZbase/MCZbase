...
<cffunction name="getUndCollObjectsHTML" access="remote" returntype="string" returnformat="plain">
	...
	<cffunction name="getSpecimens" access="remote" returntype="any" returnformat="json">
		...
		<cfquery name="qrySpecimens"  datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#" >
			SELECT DISTINCT flat.guid, flat.scientific_name,  flat.verbatim_date, flat.higher_geog, flat.spec_locality, flat.othercatalognumbers, flat.full_taxon_name
			FROM
				underscore_collection
		...
		<cfloop query="qrySpecimens">
			...
			<cfset row["HIGHER_GEOG"] = qrySpecimens.higher_geog>
			<cfset row["SPEC_LOCALITY"] = qrySpecimens.spec_locality>
				<cfset row["OTHERCATALOGNUMBERS"] = qrySpecimens.othercatalognumbers>
