...
<cffunction name=”getSearchBuilderData” displayname="Get Search Field Data" access="remote" returntype="any" returnformat="json" >
	...
	<!---  TODO: Neither argument is used in this function  ---> 
	...
	<cfif isDefined("searchText") and len(searchText) gt 0>
		<cfquery name="qryLoc" datasource="uam_god">
			select 
				#listColumns# 
			from 
				#session.flatTableName# ff, FLAT_text ft 
			WHERE 
				CONTAINS(ft.cat_num, <cfqueryparam cfsqltype="CF_SQL_TEXT" value="#searchText#">, 1) > 0 
				AND ft.collection_object_id = ff.collection_object_id
		</cfquery>
	<cfelse>
		<cfquery name="qryLoc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#">
			...
			where 
				rownum <= 50 
				and spec_locality like '%Massachusetts%'
		</cfquery>
	</cfif>
	<cfoutput>
		<cfset i = 1>
