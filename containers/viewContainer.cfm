	...
	<section class="mb-4" aria-labelledby="containerActionsHeading">
		...
		<div class="btn-toolbar" role="toolbar" aria-label="Container actions">
			...
			<cfif NOT variables.isProxyOrLeafType>
				...
				<button type="button" class="btn btn-xs btn-secondary mr-1 mb-1" onclick="openPlaceChildIntoContainerDialog(#val(getContainer.container_id)#, '#encodeForJavaScript(variables.pageTitleDisplay)#', '#encodeForJavaScript(getContainer.institution_acronym)#', 'containerViewFeedback', 'containerContentsSection_page');">Place Child into this Container</button>
			</cfif>
