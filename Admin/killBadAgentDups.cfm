	...
	<cfoutput>
		...
		<cfif #agent_relations.cnt# gt 0>
			...
			<cfquery name="relAgent" datasource="uam_god">
				SELECT * 
				FROM agent_relations 
				WHERE agent_relationship <> 'bad duplicate of'
		...
		<cfflush>
		   ...
		   <!--- Annotated agents and annotating agents ---->
			<cfquery name="annotated_agents" datasource="uam_god">
				UPDATE annotations 
				SET target_primary_key = #bads.related_agent_id# 
				WHERE target_primary_key = #bads.agent_id#
					AND upper(target_table) = 'AGENT'
			</cfquery>
			<cfquery name="annotation_reviewer_agents" datasource="uam_god">
				UPDATE annotations 
				SET reviewer_agent_id = #bads.related_agent_id# 
				WHERE reviewer_agent_id = #bads.agent_id#
			</cfquery>
			<cfquery name="annotating_agents" datasource="uam_god">
				UPDATE annotations 
				SET annotator_agent_id = #bads.related_agent_id# 
				WHERE annotator_agent_id = #bads.agent_id#
			</cfquery>
			<cfquery name="annotation_updating_agents" datasource="uam_god">
				UPDATE annotations 
				SET last_updated_by_agent_id = #bads.related_agent_id# 
				WHERE last_updated_by_agent_id = #bads.agent_id#
			</cfquery>
			got annotations<br><cfflush>
		
			<!--- Authors of taxon names --->
			<cfquery name="annotation_updating_agents" datasource="uam_god">
				UPDATE taxon_author
				SET agent_id = #bads.related_agent_id# 
				WHERE agent_id = #bads.agent_id#
			</cfquery>
		...
		<p></p>
		Anything linked above was missed and needs your attention. Otherwise, it&apos;as all cleaned up!
	</cfoutput>
</cfif>
