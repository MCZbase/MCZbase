								...
								<select id="fiscalYear" name="fiscalYear" class="mb-1 data-entry-input">
									<cfoutput>
										<cfloop array="#fiscalYears#" index="yearItem">
											<option value="#dateFormat(yearItem.beginDate, 'yyyy-mm-dd')#|#dateFormat(endDate, 'yyyy-mm-dd')#">#yearItem.label#&thinsp;(#dateFormat(yearItem.beginDate, 'yyyy-mm-dd')#/#dateFormat(yearItem.endDate, 'yyyy-mm-dd')#)</option>
										</cfloop>
									</cfoutput>
								</select>
								#beginDate# - #endDate#
