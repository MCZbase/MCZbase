...
main_plot <- ggplot(main_data, aes(x = AgentInfo, y = AdjustedCount, fill=Role)) +
  ...
  scale_y_continuous(labels = scales::comma, expand=c(0.02, 0.02), expand = c(0.02, 0.02)) +  # removed this after comma: ", expand = c(0.02, 0.02)" makes space between labels and text smaller
  theme_minimal() +
  theme(plot.title = element_text(size=unit(7,"pt"), face="bold"),
        ...
        legend.box = "vertical",
        legend.background = element_rect(margin=margin(0,0,0,0),fill=alpha('white', 0.0)), # Make the legend background transparent
        ...
        legend.title = element_text(margin = margin(0, 0, .02, 0), size=4.5), 
...
combined_plot <- main_plot + outliers_plot +
  plot_layout(guides = 'collect', widths = c(19, 2.5))
