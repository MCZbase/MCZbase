...
<!---DEV NOTE:
<p>However, it is not accessible by RStudio.</p> <p>This data comes from a temporary table in the database generated by a procedure and scheduled job which is then queried by CF to create the csv. Change this read.csv url to a local one when developing (download from testMetrics.cfm link) and import using From Text (readR). Rename to "data" to keep things consistent and change back to  '/var/www/html/arctos/metrics/datafiles/chart_data.csv' (below--uncomment) when dev and testing is complete.</p><p>#data [backward arrow] read.csv('/var/www/html/arctos/metrics/datafiles/chart_data.csv')</p>--->
			</div>
		</div>
		<div class="row">
			<div class="col-12">
